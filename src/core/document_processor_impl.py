...
class UnifiedDocumentProcessor:
    ...
    def _load_doc(self, file_path: Path) -> Tuple[str, Dict]:
        ...
        # Try antiword (Linux/Mac)
            ...
            result = subprocess.run(
                ["antiword", str(file_path)], capture_output=True, text=True, check=True
            )
            text = result.stdout
            method = "antiword"
    ...
    def chunk_text(self, text: str, metadata: Dict) -> List[DocumentChunk]:
        ...
        while start < len(text):
            ...
            if len(chunk_text) >= self.min_chunk_size:
                ...
                chunk = DocumentChunk(text=chunk_text, metadata=chunk_metadata, chunk_id=chunk_id)
                chunks.append(chunk)
                chunk_id += 1
    ...
    def process_directory(self, directory: Path) -> List[DocumentChunk]:
        ...
        logger.info(
            f"Found {len(all_files)} files: {len(pdf_files)} PDF, {len(docx_files)} DOCX, {len(doc_files)} DOC"
        )
